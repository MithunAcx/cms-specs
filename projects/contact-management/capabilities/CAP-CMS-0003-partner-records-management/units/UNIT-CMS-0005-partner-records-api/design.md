...
| `capability-design.md`'s Unified API Contract names `page`/`size` as the shared pagination convention, which conflicts with `10-platform.md`'s always-loaded, project-wide rule of cursor-only pagination ("no offset pagination anywhere") | An implementation following the capability's convention literally would violate the platform floor | `interfaces/openapi.yaml`'s two list operations (`listBrokers`, `listAgents`) use `limit`/`cursor` in, `items`/`next_cursor` out, per `10-platform.md` — the platform-wide rule is treated as controlling over the capability document's illustrative convention row, since it binds every unit rather than this capability alone; flagged here rather than silently resolved so a reviewer can object |
