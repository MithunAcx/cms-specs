...
|-------|---------|--------------------|-------------------|
| loading | Component mounts for a given `parentType`/`parentId` | Skeleton rows in place of the grid; sort/filter/pagination controls are present but disabled until the first response arrives | None — wait |
| empty — first-use | List call succeeds, `total: 0`, no filter is applied | "No activity logged yet." plus, for Editor only, the add control remains available | Add (Editor only) |
| empty — filtered-to-nothing | List call succeeds, `total: 0`, a completed/open filter or non-default sort is applied | "No entries match this filter." plus a control to clear the filter | Clear filter; Add (Editor only) |
| populated | List call succeeds, `total > 0` | The grid: task/status, note, follow-up date, entered date, completed indicator, acting user, and (Editor only) per-row edit/complete/delete controls; sort/filter/pagination controls active | Sort, filter, paginate; add/edit/complete/delete (Editor only) |
...
| disabled / no permission | Caller's session role is Viewer | Sort/filter/pagination controls are present and active; add/edit/complete/delete controls are not rendered at all — not present-and-disabled (R12/R28) | Sort, filter, paginate only |
...
| action.delete_confirm.body | This removes it from the list. It can be recovered by an administrator if needed. |
